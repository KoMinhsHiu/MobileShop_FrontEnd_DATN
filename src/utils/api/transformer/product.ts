...
export const ProductTransformer = (data: any): ProductType => {
  ...
  if (data?.psdata) {
    ...
    // Mock data structure
    return {
      title: data.name || "Sample Product",
      images: data.images?.map((img: string) => ({ src: img })) || [{ src: "/images/slider/slide1.webp" }],
      price: data.price || "29.99",
      options: data.variants?.map((variant: any) => ({
        id: String(variant.id),
        title: "Color",
        type: "color",
        items: [{ id: String(variant.id), value: variant.name, hex_value: variant.color }]
      })) || [],
